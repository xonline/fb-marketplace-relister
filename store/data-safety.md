...
The extension transmits listing content to our server when the optional AI enhancement feature is used (Provided AI mode). It also transmits listing content to Google Gemini directly in BYOK mode. Payment is handled by Stripe/ExtPay. The landing page uses GA4 analytics.
...
| **Financial and payment information** | NO (by the extension) | Payments are processed entirely by Stripe via ExtPay. The extension receives a subscription status (active/inactive) only — no card data ever reaches our code. |
...
**Conditional YES — Google (Gemini) and Stripe/ExtPay only.**
...
**Payments:**
Subscription payments are processed by Stripe via ExtPay. No card data reaches the extension or our server — only subscription status (active/inactive) is returned to the extension.
...
**YES.** All communications — to Facebook's API, to `relist.nowoly.com`, to Google Gemini, and to ExtPay/Stripe — use HTTPS (TLS). All host permissions in the manifest specify `https://` explicitly.
...
- **New in v3.7.4:** Subscription payments are handled by ExtPay/Stripe. The extension contacts `relist.nowoly.com` to verify subscription status.
